...
export const useQueryState = (
  ...
  const filters = useMemo(
    () =>
      jsonParse<Record<string, unknown>>(
        queryObj[$filters] as string | null,
        {}
      ),
    [queryObj, $filters]
  );
  ...
  const quickFilters = useMemo(
    () =>
      jsonParse<Record<string, unknown>>(
        queryObj[$quickFilters] as string | null,
        {}
      ),
  ...
  const keyword = useMemo(
    () => String(queryObj[$keyword] || ""),
    [queryObj, $keyword]
  );

  const order = useMemo(
    () => String(queryObj[$order] || ""),
    [queryObj, $keyword]
  );

  const orderBy = useMemo(
    () => String(queryObj[$orderBy] || ""),
    [queryObj, $keyword]
  );

  const tab = useMemo(
    () => String(queryObj[$tab] || initialQueryPrefix?.tab),
    [queryObj, $tab, initialQueryPrefix]
  );
